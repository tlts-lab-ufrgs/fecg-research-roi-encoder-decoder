...
for i in results_dir:
    ...
    for file in result_files:
        ...
        snr_partial = 10 * np.log10(
            np.sum(np.square(prediction_data['signal'].values)) / np.sum(np.square(testing_data[test_file]['signal'][prediction_index, :, 0]))
        )

        snr.append(snr_partial)
        
    
        r_squared_signal_partial = np.abs(
            pearsonr(testing_data[test_file]['signal'][prediction_index, :, 0], prediction_data['signal']).statistic
        )
        r_squared_mask_partial = np.abs(
            pearsonr(testing_data[test_file]['signal'][prediction_index, :, 1], prediction_data['mask']).statistic
        )
        r_squared_combined_partial = np.abs(pearsonr(
            testing_data[test_file]['roi_signal'][prediction_index], 
            prediction_data['roi_signal']).statistic
        )

        print(r_squared_mask_partial)
    ...
    # this_row.append(mean_confidence_interval(snr))
...
# fig.savefig('r03-example_index-50-abcd_dataset-model_rev0.pdf', bbox_inches='tight')
# # %%
