...
for lr in np.arange(LOWER_LIMIT_LR, UPPER_LIM_LR + LR_STEP, LR_STEP):
    
    for w_mask in np.arange(0, 1.005, 0.05):
        
        w_signal_upper_bound = 1 - w_mask
        
        for w_signal in np.arange(0, w_signal_upper_bound + 0.05, 0.05):
            ...
            for i in range(0, TOTAL_FILES, 1):
                ...
                prefix_id = f'LR_{lr}-W_MASK_{w_mask}-W_SIG_{w_signal}-LEFT_{i}'
